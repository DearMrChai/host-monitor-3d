// 一条命令的自检：语法（含 HTML 里那段模块体）→ 几何注入 → 几何对账 → CPU 并卡口径 → 综合负载口径 → 三档关联（波速/节拍拖不出反序）。
...
for (const step of ['inject-geometry.mjs', 'verify-geometry.mjs', 'verify-cpu.mjs', 'verify-load.mjs', 'verify-grade.mjs']) {
