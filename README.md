...
`设置.json`→`settings.json`、`探测记录.log`→`probe.log`）。已经跑着的 138 那台**此刻还是中文名**，
它的 `serve.mjs` 也还是旧版，所以现在一切照旧；下次推 138 时由 `00-暂存/hm3d-138-push.mjs` 里的 `RENAME`
那一步先就地改名（`Rename-Item`，内容一字不动，新名已存在就跳过）再传代码 —— 跳过这步的话新 `serve.mjs`
找不到 `devices.json`，会按内置默认现生成一份，他那几台真机的配置就当众消失。
...
能连上的人就能读走整份 `devices.json`（含明文口令），也能借 `/api/probe` 往任意主机打 SSH。
...
| 文件 | 是什么 | 看什么 |
|---|---|---|
| `monitor-wall.html` | **监控墙**：设备按四个区各自聚集（算力集群 / 终端设备区 / 储存设备区 / 网络设备区，默认以网络设备区为地图中心）+ 右侧性能看板 + 设置弹窗（设备 / 分区 / 脉冲效果 / 地面与雨 / 采集频率 五格） | 分级脉冲（脉冲效果格里有总开关）、点阵涟漪地面与二进制雨（地面与雨格里各有独立开关）、看板五节按档位增减、双击或列表里 ◎ 聚焦、SSH 实测与模拟分流（**打开页面时清单里填了地址的机器各自动抓一帧**）、每台落在哪个区、分区的名字和位置可改（存 `zones.json`）、设置那几格存 `settings.json`（改文件即改看板，页面每 5 秒读一次） |
