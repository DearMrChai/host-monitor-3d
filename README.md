...
| 文件 | 是什么 | 看什么 |
|---|---|---|
| `monitor-wall.html` | **监控墙**：设备按四个区各自聚集（算力集群 / 终端设备区 / 储存设备区 / 网络设备区）+ 右侧性能看板 + 设置弹窗（设备 / 分区 / 脉冲效果 / 性能监控 四格） | 分级脉冲、看板五节按档位增减、双击或列表里 ◎ 聚焦、SSH 实测与模拟分流、每台落在哪个区、分区的名字和位置可改（存 `分区.json`） |
